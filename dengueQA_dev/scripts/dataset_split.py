...
if __name__ == '__main__':
    ...
    parser.add_argument("--metadata", required=True, help="TSV file with descriptions about the top hits of the blast search")
    parser.add_argument("--seqcol", required=True, help="Column in metadata that contains query sequence names")
    parser.add_argument("--refcol", required=True, help="Column in metadata that contains reference sequence names")
    parser.add_argument("--typecol", required=True, help="Column in metadata that contains virus type information")
    ...
    df = load_table(blast_result)
    df = df[df[type_col] == target] # filter sequence type set from dataframe
    df.fillna('', inplace=True)
